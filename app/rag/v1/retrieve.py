...
def retrieve_legacy(q: str, k: int, require_absatz: bool = True) -> List[dict]:
    """Legacy dense-only retrieval over laws_de (e5)."""
    col = get_collection()
...
def retrieve(q: str, k: int, require_absatz: bool = True) -> List[dict]:
    """Dispatch to legacy or hybrid backend (RETRIEVER_BACKEND)."""
    backend = (RETRIEVER_BACKEND or "legacy").strip().lower()
    if backend in ("hybrid", "new", "v2"):
        from app.rag.v1.hybrid import retrieve_hybrid

        return retrieve_hybrid(q, k=k, require_absatz=require_absatz)
    return retrieve_legacy(q, k=k, require_absatz=require_absatz)


def build_context(items) -> str:
    blocks = []
    for it in items:
        absatz = (it.get("absatz") or "").strip()
        absatz_bit = f" {absatz}" if absatz else ""
        head = (
            f"[{it.get('law', '')} {it.get('paragraph', '')}{absatz_bit} "
            f"| {it['id']} | {it['source_file']}]"
        )
